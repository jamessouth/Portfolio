...
// ~alt, ~obPos~path,
@react.component
let make = (~title, ~live, ~code, ~text, ~liveAria, ~codeAria, ~i) => {
  ...
  <section
    id=title
    className={flexDir(i) ++ " flex portrait:flex-col portrait:min-h-screen landscape500:min-h-0"}>
    <div className={`bg-clean portrait:h-half-screen wdk1:h-auto w-1/2 portrait:w-auto`}>
      //   <img
      //     src={switch picState {
      //     | Error(_)
      //     | LoadingImg => ""
    ...
    <div className="w-1/2 portrait:w-auto">
      ...
      <div className="mb-10 flex justify-evenly">
        <a
          className="w-20 font-cinz text-lg border border-eerie-black rounded-full px-2 py-2 text-center hover:bg-eerie-black hover:text-white focus:bg-eerie-black focus:text-white"
